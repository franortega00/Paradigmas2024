...
testStack = [
            ...
            stackVacio,
            testF (stackS stack1AB contC), --stack1 puede cargar solo 2 contenedores y le intento cargar un tercero (falla)
            testF (netS (stackS stack1 (newC "Brunei" 0))), --stack con un container no válido-> levanta error
            testF (netS (stackS (newS 0) (newC "Brunei" 1))), --stack con un stack no válido -> levanta error
...
testVessel = [
        barcoVacioTest,
        testF (loadV barcoBA contB),--barco sobrepasado
